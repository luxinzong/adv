...
public class AdvUserServiceImpl implements iAdvUserService {
    ...
    public int deleteUserById(Integer userId){
        ...
        if(advUser == null){
            throw new com.suma.exception.UserException(ExceptionConstants.USER_EXCEPTION_ID_NOT_EXIST);
        }
        //删除对应用户信息
    ...
    public int deleteUserByIds(List<Integer> ids) {
        //判断删除的用户id是否存在，如果存在一个不存在直接返回失败
        ids.forEach(id ->{
            AdvUser advUser = advUserMapper.selectByPrimaryKey(id);
            if(advUser == null){
                throw new UserException(ExceptionConstants.USER_EXCEPTION_ID_NOT_EXIST);
            }
        });
        //删除对应用户信息
        int rows = advUserMapper.batchDeleteUser(ids);
        int deleteRows = deleteRolesByUserIds(ids);

        return rows + deleteRows;
    }
    ...
    private int deleteRolesByUserIds(List<Integer> userIds){
        return advUseRoleMapper.deleteRoleIdsByUserIds(userIds);
    }
